...
public:
	...
	void animateParticle(particle& p, float speed, float& t, controlPoint cp1, controlPoint cp2, controlPoint cp3, controlPoint cp4)
	{
		...
		auto fp = [cp1, cp2, cp3, cp4](float t) {
			return 3.0f * ((float)pow(t, 2) * (cp4.position - cp1.position + 3.0f * (cp2.position - cp3.position)) + 2.0f * t * (cp1.position - 2.0f * cp2.position + cp3.position) - cp1.position + cp2.position);
		};
